...
# Compare with theta by previous implementation
print(theta_SGD) # different result due to no control on t0 in the learning schedule.
# Prediction
...
# In[7]: UNDERFITTING AND OVERFITTING
...
# 7.4. Learning curves of Linear model
lin_reg = LinearRegression()
if let_plot:
...
# 7.6. Learning curves of Polynomial model (degree = 10)
from sklearn.pipeline import Pipeline
polynomial_regression = Pipeline([
...
# 7.8. Learning curves of Polynomial model (degree = 2)    
from sklearn.pipeline import Pipeline
polynomial_regression = Pipeline([
        ("poly_features", PolynomialFeatures(degree=2, include_bias=False)),
        ("std_scaler", StandardScaler()),
...
if let_plot:
    ...
    # Plot samples
    #plt.plot(X[y==1], log_reg.predict_proba(X[y==1])[:,1], "go", label="Iris virginica (1)", markersize=8)
    #plt.plot(X[y==0], log_reg.predict_proba(X[y==0])[:,1], "rx", label="Not Iris virginica (0)", markersize=12) 
    plt.plot(X[y==1], np.zeros(X[y==1].shape), "go", label="Iris virginica (1)", markersize=8)
    plt.plot(X[y==0], np.zeros(X[y==0].shape), "rx", label="Not Iris virginica (0)", markersize=12) 
