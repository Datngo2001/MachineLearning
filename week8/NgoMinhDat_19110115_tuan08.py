...
# => amount of x equal amount of theta
#
X_add_x0 = np.c_[np.ones((n_samples, 1)), X]
#
# Apply normal equation solution
# Return tow parameter of hypothesis function
#
...
# 1.3. Try prediction 
X_test = np.array([[1], [3], [12]]) # 3 instances
#
# add x0 = 1 to each instance
# => amount of x equal amount of theta
#
X_test_add_x0 = np.c_[np.ones((len(X_test), 1)), X_test]
y_predict = X_test_add_x0 @ theta_norm_eq

# 1.4. Plot hypothesis 
#
# plot the line with theta parameter 
#   and the line "y" in line 36 of code.
# model is good if the tow line look the same
#
if let_plot:
...
y = 2*X**2 + X + 3 + np.random.randn(m, 1)
if let_plot:
    plt.plot(X, y, "k.")
    plt.xlabel("$x_1$", fontsize=font_size)
    plt.ylabel("$y$", rotation=0, fontsize=font_size)
