...
poly_svm_1 = Pipeline([
        ("scaler", StandardScaler()),
        ("svm_clf", SVC(kernel="poly", degree=2, coef0=.001, C=5))  ]) 
poly_svm_1.fit(X, y)
...
"""
With noise=0.11:
    When gamma increased from 0.2 to 20,
    the decision boudary more acurrect and more complicate
    Because the distance to the landmark change more rapid
    
    When C increased from 0.002 to 200,
    the margin is harder 
    and make the decision boudary effected by noise,
    but with small gamma it make boudary more acurrect

With moise=0.44
    When gamma increased from 0.2 to 20,
    it make the boudary effected by noise
...
# In[7]: [SKIP] SVM OUTLIER DETECTION 
# NOTE: 
...
if let_plot:     
    ...
    id_normal = (y_pred==1)
    plt.plot(x1[id_normal], x2[id_normal], "bo", label=r"Normal samples")
    
    # Plot outliers
    id_outlier = (y_pred==-1) # -1: outlier class in ocsvm
    #id_outlier = (y_pred==0) # 0: outlier class in SVC
    plt.plot(x1[id_outlier], x2[id_outlier], "ro", label=r"Outliers")      
