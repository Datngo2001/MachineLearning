...
raw_data = pd.read_csv('NgoMinhDat_19110115_tuan04_data_2.csv')
...
dataCount = raw_data['Transmission'].value_counts()
print(dataCount) 
print('\n____________ Statistics of numeric features ____________')
dataDescribe = raw_data.describe()
print(dataDescribe)    
print('\n____________ Get specific rows and cols ____________')     
print(raw_data.loc[[0,5,20], ['Kilometers Driven', 'Selling Price']] ) # Refer using column name
print(raw_data.iloc[[0,5,20], [2, 8]] ) # Refer using column ID
 
# 3.2 Scatter plot b/w 2 features
if 0:
    raw_data.plot(kind="scatter", y="Selling Price", x="Owner", alpha=0.2)
    #plt.axis([0, 5, 0, 10000])
    plt.savefig('figures/scatter_1_feat.png', format='png', dpi=300)
    plt.show()      
if 0:
    raw_data.plot(kind="scatter", x="Selling Price", y="Car Condition", alpha=0.2)
    #plt.axis([0, 5, 0, 10000])
...
if 0:
    ...
    features_to_plot = ["Selling Price", "Car Condition", "Owner", "Kilometers Driven"]
...
if 0:
    from pandas.plotting import scatter_matrix   
    features_to_plot = ["Selling Price"]
    scatter_matrix(raw_data[features_to_plot], figsize=(12, 8)) # Note: histograms on the main diagonal
    plt.show()
...
print('\n',corr_matrix["Selling Price"].sort_values(ascending=False)) # print correlation b/w a feature and other features

# 3.7 Try combining features
raw_data["KilometersPerYear"] = raw_data["Kilometers Driven"] / raw_data["Year"] 
corr_matrix = raw_data.corr()
print(corr_matrix["Selling Price"].sort_values(ascending=False)) # print correlation b/w a feature and other features
raw_data.drop(columns = ["KilometersPerYear"], inplace=True) # remove experiment columns
#endregion
...
raw_data.drop(columns = ["Insurance"], inplace=True) 
...
if method == 1: # Method 1: Randomly select 20% of data for test set. Used when data set is large
    ...
    raw_data["KHOẢNG GIÁ"] = pd.cut(raw_data["Selling Price"],
                                    bins=[0, 272099, 355799, 503299, np.inf],
                                    #labels=["<2 tỷ", "2-4 tỷ", "4-6 tỷ", "6-8 tỷ", "8-10 tỷ", ">10 tỷ"])
                                    labels=[2,3,5,19]) # use numeric labels to plot histogram
...
train_set_labels = train_set["Selling Price"].copy()
train_set = train_set.drop(columns = "Selling Price") 
test_set_labels = test_set["Selling Price"].copy()
test_set = test_set.drop(columns = "Selling Price") 
...
num_feat_names = ['Kilometers Driven', 'Year', 'Car Condition'] # =list(train_set.select_dtypes(include=[np.number]))
cat_feat_names = ['Model', 'Owner', 'Fuel Type', 'Transmission'] # =list(train_set.select_dtypes(exclude=[np.number])) 
